    ...
    <li className="py-3 border-b border-slate-100 last:border-b-0">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right flex-shrink-0 ml-4">
          <p className="text-sm font-bold text-red-600">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : ''}</p>
...
const AdminDashboard: React.FC = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ totalUsers: 0, totalProjects: 0, completedProjects: 0, overdueTasks: 0 });
          ...
          {overdueTasksList.length > 0 ? (
            <ul className="divide-y divide-slate-100">
              {overdueTasksList.map((task) => (
                <OverdueTaskItem key={task.id} task={task} projectsMap={projectsMap} usersMap={usersMap} />
              ))}
            </ul>
          ) : (
