...
const ProjectDetail: React.FC = () => {
    ...
    const handleCreateTask = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...newTaskData,
            project: projectId,
            assign_to: newTaskData.assign_to,
                        ...
                        <div className="space-y-4">
                            {(tasksByStatus[status] || []).map(task => (
                                <TaskCard
                                    key={task.id}
                                    task={task}
                                    assigneeNames={(task.assigneeIds || []).map(id => allUsers.find(u => u.id === id)?.name || 'Unknown').filter(Boolean)}
                        ...
                        <div className="mt-1 max-h-40 overflow-y-auto border border-slate-300 rounded-md p-2 space-y-2">
                            {assignableEmployees.map(employee => (
                                <div key={employee.id} className="flex items-center">
                                    ...
                                    />
                                    <label htmlFor={`assignee-proj-${employee.id}`} className="ml-3 block text-sm text-slate-800">
                                        {employee.name}
                                    </label>
                                </div>
                            ))}
